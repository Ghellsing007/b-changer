...
export function LendBookForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error: listingError } = await supabase.from("listings").insert({
        ...
        daily_fee: Number.parseFloat(formData.dailyFee),
        max_days: Number.parseInt(formData.maxDays),
      })
      ...
      toast({
        ...
        description: "Tu libro ha sido agregado al catálogo para préstamos.",
      })
      ...
      toast({
        title: "Error",
        description: "No se pudo publicar el libro para préstamo. Inténtalo de nuevo.",
        variant: "destructive",
      })
    } finally {
      setIsLoading(false)
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Información del Libro para Préstamo</CardTitle>
        <CardDescription>Completa los detalles de tu libro para ofrecerlo en préstamo</CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="space-y-2">
              <Label htmlFor="dailyFee">Tarifa Diaria *</Label>
              <Input
                id="dailyFee"
                type="number"
                step="0.01"
                ...
                onChange={(e) => handleInputChange("dailyFee", e.target.value)}
                placeholder="0.00"
              />
            </div>

            <div className="space-y-2">
              <Label htmlFor="maxDays">Días Máximos</Label>
              <Input
                id="maxDays"
                type="number"
                min="1"
          ...
          <Button type="submit" className="w-full" disabled={isLoading}>
            {isLoading ? "Publicando..." : "Ofrecer en Préstamo"}
          </Button>
        </form>
      </CardContent>
    </Card>
